 ...
 * @Bean
 *
 * @Order(1) SecurityFilterChain authorizationServerSecurityFilterChain(HttpSecurity http)
 * throws Exception {
 *
 ...
 * new MediaTypeRequestMatcher(MediaType.TEXT_HTML) ) ) // Accept access tokens for User
 ...
 * return http.build(); }
 *
 * @Bean JdbcUserDetailsManager jdbcUserDetailsManager(DataSource dataSource) { return new
 * JdbcUserDetailsManager(dataSource); }
 *
 * @Bean ApplicationRunner usersInitializationRunner( PasswordEncoder passwordEncoder,
 * AuthorizationApiProperties properties, UserDetailsManager userDetailsManager) { return
 * args -> Stream.of(properties.users()) .peek(e ->
 ...
 *
 * @Value("${TWIS_CLIENT_REDIRECT_URI:http://127.0.0.1:8082/login/oauth2/code/spring}")
 ...
 * AuthorizationGrantType.REFRESH_TOKEN))) .redirectUri(twisRedirectUri) .scopes(scopes ->
 * scopes.addAll(Set.of("user.read", "user.write", OidcScopes.OPENID))) );
 * clients.forEach((clientId, rcb) -> { if (repository.findByClientId(clientId) == null)
