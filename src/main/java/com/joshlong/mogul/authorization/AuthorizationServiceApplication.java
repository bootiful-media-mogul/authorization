...
public class AuthorizationServiceApplication {
	...
	@Bean
	ApplicationRunner usersInitializationRunner(PasswordEncoder passwordEncoder, AuthorizationApiProperties properties,
			UserDetailsManager userDetailsManager) {
		return args -> Stream.of(properties.users())
			.peek(e -> log.info("registered new user [{}] with password [{}] and roles[{}]", e.username(), e.password(),
	...
	@Bean
	JWKSource<SecurityContext> jwkSource(@Value("${jwk.key.id:bootiful-jwk-id}") String id,
		...
		try (var publicKeyInputStream = new ByteArrayInputStream(publicKey);
			...
			var rsa = new RSAKey.Builder(rsaPublicKey).privateKey(rsaPrivateKey).keyID(id).build();
			...
			var clients = Map.of(
					...
					RegisteredClient.withId(UUID.randomUUID().toString())
						...
						.authorizationGrantTypes(
								grantTypes -> grantTypes.add(AuthorizationGrantType.CLIENT_CREDENTIALS))
