...
/*
	@Bean
	ApplicationRunner usersInitializationRunner(PasswordEncoder passwordEncoder,
												AuthorizationApiProperties properties, UserDetailsManager userDetailsManager) {
		return args -> Stream.of(properties.users())
//				.peek(e -> 		log.info("registered new user [{}] with password [{}] and roles[{}]", e.username(),
//								e.password(), Arrays.toString(e.roles()))
//				)
				.map(e -> User.builder()
...
/*	@Bean
	JdbcOAuth2AuthorizationService jdbcOAuth2AuthorizationService(JdbcOperations
																		  jdbcOperations, RegisteredClientRepository rcr) {
		return new
	...
	JWKSource<SecurityContext> jwkSource(
		...
		var publicKey =
				decoder.decode(publicKeyBase64Encoded);
		var privateKey =
				decoder.decode(privateKeyBase64Encoded);

		try (var publicKeyInputStream = new ByteArrayInputStream(publicKey); var
				privateKeyInputStream = new ByteArrayInputStream(privateKey)) {
			var rsaPrivateKey
					= RsaKeyConverters.pkcs8().convert(privateKeyInputStream);
			var rsaPublicKey =
					RsaKeyConverters.x509().convert(publicKeyInputStream);
			var rsa = new
					RSAKey.Builder(rsaPublicKey).privateKey(rsaPrivateKey).keyID(id).build();
			var
					jwk = new JWKSet(rsa);
...
@ConfigurationProperties(prefix = "mogul.authorization")
